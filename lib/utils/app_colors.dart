import 'package:flutter/material.dart';

class AppColors {
  // Brand: Terracotta #D97757 → warm amber #E8A87C gradient
  static const primaryColor1 = Color(0xFFD97757);
  static const primaryColor2 = Color(0xFFE8A87C);

  // Secondary: deep burgundy → warm rose
  static const secondaryColor1 = Color(0xFFB85C38);
  static const secondaryColor2 = Color(0xFFE07B54);

  static const whiteColor = Color(0xFFFFFFFF);
  static const blackColor = Color(0xFF1D1617);
  static const grayColor = Color(0xFF7B6F72);
  static const lightGrayColor = Color(0xFFF7F8F8);
  static const midGrayColor = Color(0xFFADA4A5);

  // Extra semantic colors
  static const successColor = Color(0xFF4CAF50);
  static const errorColor = Color(0xFFE53935);
  static const warningColor = Color(0xFFFB8C00);

  static List<Color> get primaryG => [primaryColor1, primaryColor2];
  static List<Color> get secondaryG => [secondaryColor1, secondaryColor2];
}
